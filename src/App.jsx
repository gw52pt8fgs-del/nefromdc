...
`;
  ...
  const monitorText = data.dialisi
    ...
    : "Monitorare funzione renale, diuresi ed equilibrio elettrolitico/acido-base per 48–72 ore.";
  ...
  const sglt2Text = data.sglt2
    ...
    : "";
  const diureticText = data.diuretici
    ? (data.disidratazione || data.aki || data.ipotensione
      ? "- Diuretici: sospendere temporaneamente/valutare sospensione in relazione a ipovolemia, ipotensione o AKI."
      : (data.scompenso
        ? "- Diuretici: non sospendere automaticamente; modulare in base a stato volemico, congestione e quadro emodinamico."
        : "- Diuretici: non necessaria sospensione routinaria; rivalutare in base a stato volemico e pressione arteriosa."))
    : "";
  const standardRecommendations = joinNonEmptyLines([
    `- ${hydrationText}`,
